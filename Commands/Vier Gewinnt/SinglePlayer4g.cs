...
namespace Minigame_bot.Commands.VierGewinnt
{
    public class SinglePlayer4g
    {
        ...
        public async Task StartSinglePlayer()
        {
            ...
            {
                Title = $"{p1.Name} challenges the Bot to FourInLine",
            ...
            string Title = $"{p1.Name} challenges the Bot to FourInLine";
            int Moves = 0;
            ...
            while (true)
            {
                ...
                int[] BotzugArr = Botzug(Spielfeld);
                Spielfeld[BotzugArr[0], BotzugArr[1]] = aiemote;
                Moves++;
                Desc = Concat(Spielfeld);
            }

            //Ab hier Ende der while schleife --> Einer der Spieler hat gewonnen
            Playeractive = Playeractive == "Player 1" ? p1.Name : ai.Name;
            await msg.ModifyAsync(x =>
            {
                DiscordEmbedBuilder embed = new();
        ...
        private int[] Botzug(string[,] Spielfeld)   //Bot move
        {
            while (true)
            {
                Random rnd = new();
                var x = rnd.Next(0, Spielfeld.GetLength(1));
                if (Check_if_full(Spielfeld, x))
                {
                    continue;
                }
                var y = GetPos(Spielfeld, x);
                return new int[] { y, x };
            }
        }
